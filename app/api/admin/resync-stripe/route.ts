...
export async function POST() {
  try {
    ...
    const subResult = { checked: 0, updated: 0, revertedToPending: 0, ghostSubsCleared: 0, errors: [] as string[] }
      ...
      try {
        ...
        if (stripeSubscriptionId) {
          ...
          if (stripeSubMissing) {
            ...
            } else if (sub.payment_status === 'paid' || sub.payment_status === 'processing') {
              updateData.payment_status = 'pending'
            }
        ...
        // B) Nur Checkout-Session → Session-Status prüfen
          ...
          // → zurück auf 'pending', damit Coach erinnern kann. Greift den Lambinot-Kastrati-Fall ab.
            ...
            piStatus === 'canceled' || piStatus === 'requires_payment_method'
          ) {
            if (sub.status === 'pending' && sub.payment_status === 'processing') {
              updateData.payment_status = 'pending'
        ...
        //    (nötig für Abos, wo RLS die ID-Writes verwarf)
          ...
          for (const customer of customers.data) {
            ...
            const realProcessingSession = sessions.data.find(s =>
              s.status === 'complete' && s.payment_status === 'unpaid' && s.payment_intent
            )
            ...
            if (paidSession) {
              ...
              const piId = typeof realProcessingSession.payment_intent === 'string'
              ...
              if (piId) {
                ...
                if (pi.status === 'processing' || pi.status === 'requires_action') {
                  updateData.stripe_customer_id = customer.id
                  updateData.stripe_checkout_session_id = realProcessingSession.id
          ...
          if (!matched && sub.status === 'pending' && sub.payment_status === 'processing') {
            updateData.payment_status = 'pending'
            subResult.revertedToPending++
          }
        }
        ...
        if (
          ...
          try {
            ...
            for (const cid of customerIds) {
              const pis = await stripe.paymentIntents.list({ customer: cid, limit: 20 })
        ...
        const endStatus = (updateData.status as string | undefined) ?? (sub.status as string)
        const endPaymentStatus = (updateData.payment_status as string | undefined) ?? (sub.payment_status as string)
        if (endStatus === 'pending' && endPaymentStatus === 'processing') {
          try {
            ...
            for (const cid of customerIds) {
              const pis = await stripe.paymentIntents.list({ customer: cid, limit: 20 })
              if (pis.data.some(p =>
                p.status === 'processing' || p.status === 'succeeded' || p.status === 'requires_action'
              )) {
                hasRealAttempt = true
                break
              }
            ...
            console.warn(`Sub ${sub.id}: SEPA-Realitäts-Check fehlgeschlagen:`, e)
          }
        }
        ...
        if (
          ...
          try {
            ...
            if (stripeCustomerId) {
              const customerSubs = await stripe.subscriptions.list({ customer: stripeCustomerId, status: 'all', limit: 10 })
              const active = customerSubs.data.find(s => s.status === 'active' || s.status === 'trialing' || s.status === 'past_due')
              if (active) {
            ...
            if (!foundActive && memberEmail) {
              const customers = await stripe.customers.list({ email: memberEmail, limit: 5 })
              for (const c of customers.data) {
                const customerSubs = await stripe.subscriptions.list({ customer: c.id, status: 'all', limit: 10 })
                const active = customerSubs.data.find(s => s.status === 'active' || s.status === 'trialing' || s.status === 'past_due')
                if (active) {
            ...
            console.warn(`Sub ${sub.id}: Stripe-Lookup für Phantom-Active fehlgeschlagen:`, e)
          }
        }
